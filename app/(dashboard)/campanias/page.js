...
export default function CampaniasPage() {
  ...
  const handleEjecutar = async (campania) => {
    if (!confirm(`Esta seguro de ejecutar la campania "${campania.nombre}"? Esto ejecutara llamadas para todas las bases asignadas.`)) return;

    setEjecutando(true);
    try {
      // Obtener bases de la campania y tipificaciones
      const [basesRes, tipificacionRes] = await Promise.all([
        apiClient.get(`/crm/campanias/${campania.id}/bases`),
      ...
      // Ejecutar llamadas para cada base
      for (const base of bases) {
        const detallesRes = await apiClient.get(`/crm/bases-numeros/${base.id_base_numero}/detalles`);
        const numeros = detallesRes?.data;
        if (!numeros) continue;
          ...
          numeros.map(async (num) => {
            ...
            if (!persona) {
              const personaRes = await apiClient.post('/crm/persona', {
                ...
                id_estado: 1,
                id_tipo_persona: num.id_tipo_persona || 1,
              });
      ...
      // Registrar ejecuciones en backend
  ...
  // Obtener bases no asignadas para el modal de gestion (filtradas por formato de la campania)
  const basesNoAsignadas = basesDisponibles.filter(
    base => !basesAsignadas.some(ba => ba.id_base_numero === base.id) &&
      (!selectedCampania?.id_formato || base.id_formato === selectedCampania.id_formato)
  );
  ...
  return (
    <div className="space-y-6">
              ...
              >
                ...
                <TableCell>
                  {campania.formato_nombre ? (
                    <Badge variant="secondary" className="text-[10px] bg-amber-50 text-amber-700 border border-amber-200/50 gap-1">
      ...
      {/* ========== MODAL: EJECUCIONES ========== */}
      <Dialog open={showEjecucionesModal} onOpenChange={(open) => { if (!open) setShowEjecucionesModal(false); }}>
