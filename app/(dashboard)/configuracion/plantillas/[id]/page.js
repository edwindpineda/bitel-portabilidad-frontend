...
export default function EditarPlantillaPage() {
  ...
  const loadData = async () => {
    try {
      ...
      setFormData({
        ...
        prompt: normalizeNewlines(plantilla.prompt || '')
      });

      // Cargar campos del formato si existe
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSaving(true);
      await apiClient.put(`/crm/plantillas/${plantillaId}`, formData);
      router.push('/configuracion/plantillas');
    } catch (error) {
      console.error('Error al actualizar plantilla:', error);
      alert(error.msg || 'Error al actualizar plantilla');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">Modificar plantilla de prompt existente</p>
      </div>
