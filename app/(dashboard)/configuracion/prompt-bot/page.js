...
export default function PromptBotPage() {
  ...
  const [toolNombre, setToolNombre] = useState(null);
  ...
  const fetchToolDefinitions = async () => {
    try {
      const response = await apiClient.get('/crm/tool-definitions');
      if (response.data) {
        setToolDefinitions(response.data.tools || []);
        setToolNombre(response.data.tool_nombre || null);
      }
    } catch (error) {
      console.error('Error al cargar tool definitions:', error);
    } finally {
      setToolsLoaded(true);
    }
  };
        ...
        <div className="px-4 py-2 bg-blue-50 border-b border-blue-100 flex-shrink-0">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center flex-wrap gap-2 text-sm">
              ...
              <code className="px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded text-xs font-mono">{'{{datos_cliente}}'}</code>
              <code className="px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded text-xs font-mono">{'{{fecha_hora_actual}}'}</code>
            </div>
          </div>
        </div>
        ...
        {toolsLoaded && (
          <div className="px-4 py-2 bg-purple-50 border-b border-purple-100 flex-shrink-0">
            <div className="flex items-center space-x-3">
              ...
              <div className="flex items-center flex-wrap gap-2 text-sm">
                <span className="text-purple-800 font-medium">Tools{toolNombre ? ` (${toolNombre})` : ''}:</span>
                {toolDefinitions.length > 0 ? (
                  ...
                  <span className="text-purple-500 text-xs italic">No hay tools configurados para esta empresa</span>
                )}
              </div>
            </div>
          </div>
