...
export const whatsappTemplatesService = {
  ...
  /**
   * Envía una plantilla a un número (usa bitel-portabilidad API que registra en BD)
   */
  enviar: async (phone, templateName, language = 'es', components = []) => {
    try {
      const response = await apiClient.post('/plantillas-whatsapp/enviar', {
        phone,
...
export const whatsappEmbeddedService = {
   ...
   */
  procesarToken: async (accessToken, eventType = 'FINISH', idPlataforma = 4) => {
    try {
   ...
   */
  obtenerConfiguracion: async (idPlataforma = 4) => {
    try {
   ...
   */
  desconectar: async (idPlataforma = 4) => {
    try {
   ...
   */
  verificarEstado: async (idPlataforma = 4) => {
    try {
   ...
   */
  suscribirWebhook: async (idPlataforma = 4) => {
    try {
   ...
   */
  suscribirCoexistence: async (idPlataforma = 4) => {
    try {
   ...
   */
  sincronizarSMB: async (idPlataforma = 4, syncType = 'all') => {
    try {
  ...
  /**
   * Guarda la configuracion de WhatsApp en la BD local (configuracion_whatsapp)
   * Hace upsert: si ya existe para la empresa, actualiza; si no, crea.
   */
  guardarConfiguracionLocal: async (idEmpresa, data) => {
    try {
      // Verificar si ya existe configuracion para esta empresa
      const existing = await apiClient.get(`/crm/configuracion-whatsapp/empresa/${idEmpresa}`);

      if (existing?.data?.id) {
        // Actualizar configuracion existente
        const response = await apiClient.put(`/crm/configuracion-whatsapp/${existing.data.id}`, {
          app_id: data.app_id || null,
          numero_telefono_id: data.phone_number_id || null,
          clave_secreta: data.app_secret || null,
          token_whatsapp: data.access_token || null,
          waba_id: data.waba_id || null,
          phone_number: data.phone_number || null,
          token_expiration: data.token_expiration || null,
        });
        return response;
      } else {
        // Crear nueva configuracion
        const response = await apiClient.post('/crm/configuracion-whatsapp', {
      ...
      // Si el GET falla con 404, crear nueva configuracion
      ...
      console.error('Error al guardar configuracion local:', error);
      throw error;
    }
  },
};
