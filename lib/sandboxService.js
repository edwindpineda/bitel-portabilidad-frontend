...
export const getConfiguracionById = async (id) => {
  const response = await apiClient.get(`/sandbox/configuracion/${id}`);
...
export const sendMessageWebhookMock = async (idChat, data) => {
  return apiClient.post(`/sandbox/chats/${idChat}/messages/webhook-mock`, data);
};
