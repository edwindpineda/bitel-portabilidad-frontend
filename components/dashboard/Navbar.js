...
export default function Navbar() {
  ...
  return (
    <header
      ...
      style={{
        background: '#0F172A',
        boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.3), 0 4px 20px -2px rgba(20, 184, 166, 0.08)',
      }}
    >
      {/* Search Bar */}
      <div className="flex-1 max-w-lg">
        <div className="relative group">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-teal-400/60 group-focus-within:text-teal-300 transition-colors" />
          <input
            type="search"
            placeholder="Buscar conversaciones, clientes..."
            className="block w-full pl-10 pr-4 py-2 bg-white/[0.06] border border-white/[0.08] rounded-xl text-sm text-white/90 placeholder-slate-400/50 focus:outline-none focus:bg-white/[0.1] focus:border-teal-500/40 focus:ring-1 focus:ring-teal-500/20 transition-all duration-200"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 ml-6">
        ...
        <Button
          variant="ghost"
          size="icon"
          className="relative h-9 w-9 rounded-xl text-teal-300/60 hover:text-white hover:bg-white/[0.08] transition-all duration-200"
        >
            ...
            >
              <Avatar className="h-8 w-8 rounded-lg ring-2 ring-white/[0.1]">
                <AvatarFallback
                  ...
                  style={{ background: 'linear-gradient(135deg, #14B8A6 0%, #22D3EE 100%)' }}
                >
              ...
              <div className="text-left hidden md:block">
                ...
                <p className="text-[11px] text-teal-300/50 leading-tight">{user.rol_nombre}</p>
              </div>
              <ChevronDown className="h-3.5 w-3.5 text-teal-300/50 hidden md:block" />
            </Button>
          </DropdownMenuTrigger>
